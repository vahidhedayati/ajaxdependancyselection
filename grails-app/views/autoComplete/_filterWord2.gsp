...
function ${params.id}AppendValue(event) {
	$('#${params.id}${params.hidden}').val(event.value);
		...
		$.getJSON('${createLink(controller:"${params?.acontroller}", action: "${params?.filteraction2}")}?term=' + event.value + '&id=${params.id}&prevValue='+gotprevId+'&value=${params?.triggerWord}&bindid='+gotprevId+'&collectField=${params?.collectField}&searchField=${params?.searchField}&domain=${params?.domain}&domain2=${params?.domain2}&filterbind=${params.filterbind}&filter=${params.filter}&filter2=${params.filter2}',function(data){
			var e=data;
			if (e) { 
