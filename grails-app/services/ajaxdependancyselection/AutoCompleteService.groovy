...
class AutoCompleteService {
	...
	def autocomplete (params) {
		if (verifySecurity(params.domain, params.searchField, params.collectField)) {
	...
	def selectSecondary(params) {
		def primarySelectList=[]
		if (verifySecurity(params.domain2, params.searchField, params.collectField)) {
	...
	private Boolean verifySecurity (String domainClass, String search, String collection) {
		...
		if (config) {
			...
			if (config.security && config.security == 'enabled') {
				if (clist.dc && clist.dc.contains(domainClass))  {
					clist.each {cl ->
						println "----> ${clist.dc}"
						if ((cl.dc == domainClass) && (cl.search && cl.search == search) && (cl.collect && cl.collect == collection)) {
							status = true
						}
					}
				}
